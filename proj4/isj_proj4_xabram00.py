...
def quizemaker(qpool, index, altcodes): 
    """universal additional function for quiz generating 
    """
    quizTemp2=[] #temporary Array for quiz
    for a in index:
        try:
            quizTemp1=[]    #temporary Array for question
            quizTemp1.extend(list(qpool[a]))  #question copy
            lr_alt=list(range(len(altcodes)))  #index array for altcodes
            lr_quizTemp1=list(range(len(quizTemp1[1]))) #index array for questions
            lr=min(lr_alt,lr_quizTemp1)      #max lenth of the new q.array
            for x in lr_quizTemp1:   #reduce the number of questions according        
                if lr_quizTemp1>lr_alt:  #to the number of alternative codes
                    quizTemp1[1].pop()
                    lr_quizTemp1=list(range(len(quizTemp1[1]))) #new number of questions
            for b,c in zip(lr,altcodes): #for b,c (example: for [0,1,2] and [A,B,C] = (0,A)(2,B)(3,C)))
                if ":" not in quizTemp1[1][b]: #only new questions
                    quizTemp1[1][b]=c+": "+quizTemp1[1][b] #Question -> A: Question   
            quizTemp2.append(tuple(quizTemp1))
        except IndexError:
...
def gen_quiz(qpool, *index, altcodes=None, quiz=None): 
    """main function from task 
    """
    #entry examples:
    #gen_quiz(test_qpool4, 0, 1, -1)
    if not altcodes and not quiz:
        alph=list(string.ascii_uppercase)
        return (quizemaker(qpool, index, alph))
    ...
    if quiz and not altcodes: 
        if not index: 
            ...
            for a in index:
                try:
                    quizTemp2.append(tuple(qpool[a]))
                except IndexError:
                    print("Ignoring index %s - list index out of range"%(a))
            return quizTemp2
    #entry examples:
    #gen_quiz(test_qpool1, -2, 0, altcodes = ('10', '20', '30'), quiz = existing_quiz1)
    #gen_quiz(test_qpool2, -2, 0, altcodes = ('1', '2', '3'), quiz = existing_quiz2)
    #test_qpool3 (rekursive)
    else:
